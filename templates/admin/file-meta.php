...
<h2>
	<?php echo __('File meta settings', 'nm-wpcomments')?>
</h2>
  ...
  <div id="billing-f">
    ...
    <div id="billing-section-fields">
    	...
    	<div class="form-meta-bttons">
			<p>
		<?php _e('select input type below and drag it on right side. Then set more options', 'nm-wpcomments')?>
		</p>
		...
		<div class="form-meta-setting postbox-container">
			...
			<div class="postbox">
				<h3>
					<span style="float: left"><?php _e('Drage form fields here', 'nm-wpcomments')?></span>
					<span style="float: right"><span style="float: right"
						title="<?php _e('Collapse all', 'nm-wpcomments')?>"
						class="ui-icon ui-icon-circle-triangle-n"></span><span
						title="<?php _e('Expand all', 'nm-wpcomments')?>"
				...
				<div class="inside" style="background-color: #fff;">
					...
					<?php render_existing_form_meta($comment_meta, $nmwpcomment -> inputs)?>
					</ul>
				</div>
			</div>
			
			<p>
    		<button class="button-primary button" onclick="save_file_meta()"><?php _e('Save all changes', 'nm-wpcomments')?></button>
...
<div id="remove-meta-confirm"
	title="<?php _e('Are you sure?', 'nm-wpcomments')?>">
	<p>
	...
	switch ($type) {
		...
		case 'paired' :
			...
			if($value){
				foreach ($value as $option){
					...
					$html_input .= '<input type="text" name="options[option]" value="'.$option['option'].'" placeholder="'.__('option','nm-wpcomments').'">';
					$html_input .= '<input type="text" name="options[price]" value="'.$option['price'].'" placeholder="'.__('price (if any)','nm-wpcomments').'">';
					$html_input	.= '<img class="add_option" src="'.$nmwpcomment->plugin_meta['url'].'/images/plus.png" title="add rule" alt="add rule" style="cursor:pointer; margin:0 3px;">';
					$html_input	.= '<img class="remove_option" src="'.$nmwpcomment->plugin_meta['url'].'/images/minus.png" title="remove rule" alt="remove rule" style="cursor:pointer; margin:0 3px;">';
					$html_input .= '</div>';
				}
			}else{
				$html_input .= '<div class="data-options" style="border: dashed 1px;">';
				$html_input .= '<input type="text" name="options[option]" placeholder="'.__('option','nm-wpcomments').'">';
				$html_input .= '<input type="text" name="options[price]" placeholder="'.__('price (if any)','nm-wpcomments').'">';
				$html_input	.= '<img class="add_option" src="'.$nmwpcomment->plugin_meta['url'].'/images/plus.png" title="add rule" alt="add rule" style="cursor:pointer; margin:0 3px;">';
				$html_input	.= '<img class="remove_option" src="'.$nmwpcomment->plugin_meta['url'].'/images/minus.png" title="remove rule" alt="remove rule" style="cursor:pointer; margin:0 3px;">';
				$html_input .= '</div>';
			}
		...
		case 'html-conditions' :
			...
			if($value){
				...
				$html_input .= '<option '.$visibility_show.'>'.__('Show','nm-wpcomments').'</option>';
				$html_input .= '<option '.$visibility_hide.'>'.__('Hide', 'nm-wpcomments').'</option>';
				$html_input	.= '</select> ';
					
					
				$html_input .= __('only if', 'nm-wpcomments');
				...
				$html_input 	.= '<option '.$bound_all.'>'.__('All','nm-wpcomments').'</option>';
				$html_input .= '<option '.$bound_any.'>'.__('Any', 'nm-wpcomments').'</option>';
				$html_input	.= '</select> ';

				$html_input .= __(' of the following matches', 'nm-wpcomments');
				...
				foreach ($value['rules'] as $condition){
					...
					$html_input .= '<br><strong>'.__('Rule # ', 'nm-wpcomments') . $rule_i++ .'</strong><br>';
					...
					$html_input	.= '<option '.$operator_is.'>'.__('is','nm-wpcomments').'</option>';
					$html_input .= '<option '.$operator_not.'>'.__('not', 'nm-wpcomments').'</option>';
					$html_input .= '<option '.$operator_greater.'>'.__('greater then', 'nm-wpcomments').'</option>';
					$html_input .= '<option '.$operator_less.'>'.__('less then', 'nm-wpcomments').'</option>';
					$html_input	.= '</select> ';
					...
					$html_input	.= '<img class="add_rule" src="'.$nmwpcomment->plugin_meta['url'].'/images/plus.png" title="add rule" alt="add rule" style="cursor:pointer; margin:0 3px;">';
					$html_input	.= '<img class="remove_rule" src="'.$nmwpcomment->plugin_meta['url'].'/images/minus.png" title="remove rule" alt="remove rule" style="cursor:pointer; margin:0 3px;">';
					$html_input .= '</div>';
				...
				$html_input	 = '<select name="condition_visibility">';
				$html_input .= '<option>'.__('Show','nm-wpcomments').'</option>';
				$html_input .= '<option>'.__('Hide', 'nm-wpcomments').'</option>';
				$html_input	.= '</select> ';
					
				$html_input	.= '<select name="condition_bound">';
				$html_input .= '<option>'.__('All','nm-wpcomments').'</option>';
				$html_input .= '<option>'.__('Any', 'nm-wpcomments').'</option>';
				$html_input	.= '</select> ';
				...
				$html_input .= '<select name="condition_operators">';
				$html_input	.= '<option>'.__('is','nm-wpcomments').'</option>';
				$html_input .= '<option>'.__('not', 'nm-wpcomments').'</option>';
				$html_input .= '<option>'.__('greater then', 'nm-wpcomments').'</option>';
				$html_input .= '<option>'.__('less then', 'nm-wpcomments').'</option>';
				$html_input	.= '</select> ';
		...
		case 'pre-images' :
			...
			$html_input	.= '<input name="pre_upload_image_button" type="button" value="'.__('Select/Upload Image', 'nm-wpcomments').'" />';
			// nm_personalizedproduct_pa($value);
			if ($value) {
